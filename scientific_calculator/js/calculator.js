...
function saveToHistorial() {
    ...
    if (resultH != "Error" && resultH !="undefined" && resultH !="function sqrt() { [native code] }" && resultH != "NaN") {
        ...
        document.querySelector("#historial").insertAdjacentHTML("afterbegin", `<section class='hist alert alert-primary'> ${equation} = ${resultH} </section>`);
